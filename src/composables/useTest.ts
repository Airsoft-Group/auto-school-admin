...
interface ExamState {
    ...
    questionIds: string[]
    examType: ExamType
    questionSlots?: [number, QuestionWithShuffledAnswers][]
    answerShuffles?: [number, string[]][]
}
...
export function useExam(questionsData: Question[], examType: ExamType = 'ticket', examId?: string, sessionId?: string, timeLimitMinutes?: number) {
    ...
    const allQuestions = ref<Question[]>(questionsData || [])
    const questions = ref<QuestionWithShuffledAnswers[]>([])
    const questionSlots = ref<Map<number, QuestionWithShuffledAnswers>>(new Map())

    // Har bir index uchun javoblarning shuffle tartibini saqlash
    const answerShuffles = ref<Map<number, string[]>>(new Map())

    const STORAGE_KEY = `exam_state_${examType}_${allQuestions.value[0]?.id || 'default'}`

    // Javoblarni shuffle qilish funksiyasi
    ...
    const initializeQuestions = () => {
        ...
        selected.forEach((question, index) => {
            ...
            const answerIds = shuffledAnswers.map((a) => a.id)

            questionSlots.value.set(index, {
                ...question,
                shuffledAnswers,
            })
            answerShuffles.value.set(index, answerIds)
        })
    ...
    const loadState = (): ExamState | null => {
        try {
            ...
            // Shuffle holatini qayta yuklash
            if (state.answerShuffles) {
                answerShuffles.value = new Map(state.answerShuffles)

                // Har bir savol uchun shuffled answers ni qayta tiklash
                questionSlots.value.forEach((question, index) => {
                    const savedOrder = answerShuffles.value.get(index)
                    if (savedOrder) {
                        const orderedAnswers = savedOrder.map((id) => question.answers.find((a) => a.id === id)).filter(Boolean) as Answer[]
    ...
    const saveState = () => {
        ...
        try {
            const state: ExamState = {
                ...
                examType: examTypeRef.value,
                questionSlots: Array.from(questionSlots.value.entries()),
                answerShuffles: Array.from(answerShuffles.value.entries()),
            }
            localStorage.setItem(STORAGE_KEY, JSON.stringify(state))
        } catch (error) {
            console.error('Error saving exam state:', error)
    ...
    watch(
        [currentIndex, currentPage, selectedAnswer, timeLeft, finished, userAnswers, questionSlots, answerShuffles],
        () => {
            saveState()
        },
        { deep: true }
    ...
    const currentQuestion = computed(() => {
        ...
        // Shuffled answers qaytarish
        return {
            ...question,
            answers: question.shuffledAnswers || question.answers,
        }
    })
    ...
    function jumpToQuestion(index: number) {
        if (index >= 0 && index < questionLimit) {
            ...
            if (isAnswered) {
                console.log('✅ Question already answered - keeping same question and shuffle')
                const savedAnswer = userAnswers.value.get(index)
                selectedAnswer.value = savedAnswer?.answerId || null
            } else {
                console.log('🔄 Question not answered - reshuffling answers only')

                // Agar bu index uchun savol mavjud bo'lsa
                let question = questionSlots.value.get(index)

                if (question) {
                    // Faqat javoblarni yangi shuffle qilish
                    const newShuffledAnswers = shuffleAnswers(question.answers)
                    const answerIds = newShuffledAnswers.map((a) => a.id)
